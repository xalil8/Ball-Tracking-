...
class ControlledLaser(object):
	...
	def step(self, image, speed):
		...
		xspd = np.clip(np.rint(np.random.normal(speed[0], self.noise_sigma[0], 1)), -self.max_speed[0], self.max_speed[0])

		yspd = np.clip(np.rint(np.random.normal(speed[1], self.noise_sigma[1], 1)), -self.max_speed[1], self.max_speed[1])

		self.__x_center = int(np.clip(self.__x_center + (np.sign(xspd) * np.sign(self.__x_spd) >= 0) * xspd, 0, self.xmax))
		self.__y_center = int(np.clip(self.__y_center + (np.sign(yspd) * np.sign(self.__y_spd) >= 0) * yspd, 0, self.ymax))
